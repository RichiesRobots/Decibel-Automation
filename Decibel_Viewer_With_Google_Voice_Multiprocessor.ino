...
//Added by Richies Robots 

#define USE_GOOGLE_VOICE YES  // Yes if to use Google Voice

String GOOGLE_SPEAKER_NAME = "Tuinhuis speaker";  //Name of Google Speaker

//How often should google check and message about the noise (minutes)

volatile double deb_counter = 0;  // For tracking decibels
volatile double deb_loop_counter = 0;

double sampleTime = 10; // how long to sample decibels for and then act (in seconds)


double maxdBA = 0;  //Keep track on minimum and max levels to display
double mindBA = 120;


#define BETWEEN(value, min, max) (value <= max && value >= min) //Function to check between two values for the decibels

#define WIFI_SSID "RHomeOutPersonal"
#define WIFI_PW "QuintenJennifer648!"


  #if (USE_GOOGLE_VOICE == YES)
    #include <WiFi.h> 
...
void setup() {
  ...
  //Richies Robotos Addition, connect to wifi if using the Google Speaker option
    #if (USE_GOOGLE_VOICE == YES)
         initializeWiFi("Connecting to "); 
         Serial.println("Connecting to Google Home...");
         if (ghn.device("Tuinhuis speaker", "en") != true) 
            {
         ...
         ghn.notify("Google Speaker Connected");
     


  //Added by Richies Robots - Creating the process to talk to Google on the second core as to not stop Decibel measurements and to time independently

         xTaskCreatePinnedToCore(
                    GoogleWarningCheck,   /* Function to implement the task */
                    "GoogleWarningCheck", /* Name of the task */
  ...
  while (xQueueReceive(samples_queue, &q, portMAX_DELAY)) {
    ...
    if (Leq_samples >= SAMPLE_RATE * LEQ_PERIOD) {
      ...
      //Richies Robots, set Max and Min to display, but do not show if it shows infinity due to acoustic overload or below noise floor measurement
      if(Leq_dB > maxdBA && Leq_dB != INFINITY){
        maxdBA = Leq_dB;
      }
      if(Leq_dB < mindBA && Leq_dB != INFINITY){
        mindBA = Leq_dB;
      }
      ...
      //Richies Robots Addition, display the max and min decibels only if screen is 128 x 64

      #if (OLED_GEOMETRY == GEOMETRY_128_64)

         display.drawString(0, 21, String(maxdBA, 1) + " " + DB_UNITS + " Max");

         display.drawString(0, 42, String(mindBA, 1) + " " + DB_UNITS + " Min");
    ...
    //Richies Robots Addition, check if we use Google Voice and run function
    #if (USE_GOOGLE_VOICE == YES)
     deb_counter = deb_counter + Leq_dB;
     deb_loop_counter++;
    #endif
...
//For reconnecting when a network request is 
...
//function check and messaging at certain frequencies
void GoogleWarningCheck(void * pvParameters){

double Dec;
for(;;){

Dec = deb_counter / deb_loop_counter;

Serial.println(Dec);

  checkReconnectWifi();

      //here set a message depending on decibel level eg: between 70-100 decibels then "be quite!"
 
      if(BETWEEN(Dec, 80, 100)){
         ghn.notify("Richie, please speak more softly!");
      }

deb_counter = 0; //reset after to counter Googles Voice
deb_loop_counter = 0;
